...
export class TextCleaner {
  ...
  constructor() {
    for (let i = 0; i < this.toReplace.length; i++) {
      //We know this will never be null since the hard coded arrays are the same length, so this rule is OK to disable
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
      this.charMap[this.toReplace[i]] = this.replaceWith[i]!;
    }
    ...
    this.charMapExpression = new RegExp(this.toReplace.join('|'), 'g');
  }
